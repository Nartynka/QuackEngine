...
namespace Quack
{
	...
	void UI::RenderMenu()
	{
		...
		if (ImGui::CollapsingHeader("Spawn entities", ImGuiTreeNodeFlags_None))
		{
			ImGui::SeparatorText("Spawn stack of:");
			...
			{
				context->SpawnStackOfCubes(size, pos);
			}
			ImGui::SameLine();
			if (ImGui::Button("Spheres", ImVec2(buttonWidth, 0.0f)))
			{
				context->SpawnStackOfSpheres(size, pos);
			}

			ImGui::SeparatorText("Manage entities:");
			ImGui::Text("Entity count: %i", context->GetEntityCount());
			if (ImGui::Button("Clear all entities", { -1.f, 0.0f }))
			{
		...
		// @TODO: UI shouldn't be responsible for creating entities. It should just call a function in the scene that creates an entity with the given parameters
		if (ImGui::CollapsingHeader("Create entity", ImGuiTreeNodeFlags_None))
		{
